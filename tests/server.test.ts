...
Deno.test("Server Tools: get_weather tool registered", async () => {
  const result = listTools();
  const toolNames = result.tools.map((tool) => tool.name);
  assert(toolNames.includes("meteo__get_weather"));
});

Deno.test("Server Tools: get_snow_conditions tool registered", async () => {
  const result = listTools();
  const toolNames = result.tools.map((tool) => tool.name);
  assert(toolNames.includes("meteo__get_snow_conditions"));
});

Deno.test("Server Tools: get_weather_alerts tool registered", async () => {
  const result = listTools();
  const toolNames = result.tools.map((tool) => tool.name);
  assert(toolNames.includes("meteo__get_weather_alerts"));
});

Deno.test("Server Tools: get_historical_weather tool registered", async () => {
  const result = listTools();
  const toolNames = result.tools.map((tool) => tool.name);
  assert(toolNames.includes("meteo__get_historical_weather"));
});

Deno.test("Server Tools: get_marine_conditions tool registered", async () => {
  const result = listTools();
  const toolNames = result.tools.map((tool) => tool.name);
  assert(toolNames.includes("meteo__get_marine_conditions"));
});

Deno.test("Server Tools: get_comfort_index tool registered", async () => {
  const result = listTools();
  const toolNames = result.tools.map((tool) => tool.name);
  assert(toolNames.includes("meteo__get_comfort_index"));
});

Deno.test("Server Tools: get_astronomy tool registered", async () => {
  const result = listTools();
  const toolNames = result.tools.map((tool) => tool.name);
  assert(toolNames.includes("meteo__get_astronomy"));
});

Deno.test("Server Tools: search_location_swiss tool registered", async () => {
  const result = listTools();
  const toolNames = result.tools.map((tool) => tool.name);
  assert(toolNames.includes("meteo__search_location_swiss"));
});

Deno.test("Server Tools: compare_locations tool registered", async () => {
  const result = listTools();
  const toolNames = result.tools.map((tool) => tool.name);
  assert(toolNames.includes("meteo__compare_locations"));
});

Deno.test("Server Tools: tool count is 11", async () => {
  const result = listTools();
...
Deno.test("Server Resources: weather codes resource registered", async () => {
  const result = listResources();
  const resourceUris = result.resources.map((r) => r.uri);
  assert(resourceUris.includes("weather://codes"));
});

Deno.test("Server Resources: weather parameters resource registered", async () => {
  const result = listResources();
  const resourceUris = result.resources.map((r) => r.uri);
  assert(resourceUris.includes("weather://parameters"));
});

Deno.test("Server Resources: resource count is 4", async () => {
  const result = listResources();
...
Deno.test("Server Prompts: ski trip weather prompt registered", async () => {
  const result = listPrompts();
  const promptNames = result.prompts.map((p) => p.name);
  assert(promptNames.includes("meteo__ski-trip-weather"));
});

Deno.test("Server Prompts: plan outdoor activity prompt registered", async () => {
  const result = listPrompts();
  const promptNames = result.prompts.map((p) => p.name);
  assert(promptNames.includes("meteo__plan-outdoor-activity"));
});

Deno.test("Server Prompts: weather aware travel prompt registered", async () => {
  const result = listPrompts();
  const promptNames = result.prompts.map((p) => p.name);
  assert(promptNames.includes("meteo__weather-aware-travel"));
});

Deno.test("Server Prompts: prompt count is 3", async () => {
  const result = listPrompts();
  assertEquals(result.prompts.length, 3);
});

Deno.test("Server Prompts: ski trip weather prompt content", async () => {
...
Deno.test("Server Prompts: plan outdoor activity prompt content", async () => {
  const result = await getPrompt({
    params: {
...
Deno.test("Server Prompts: weather aware travel prompt content", async () => {
  const result = await getPrompt({
    params: {
